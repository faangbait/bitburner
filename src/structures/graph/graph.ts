...
class Graph<T> {
  ...
  // FORMAT: <first vertex u>' '<second vertex v>' ' <edge w>
  static create = (ns: NS, file: string) => {
  ...
  static createListAdj = (ns: NS, file: string) => {
    const g = new Graph<string>();
    const data = ns.read(file);
    let line = "";
    let split: string[] = [];
  ...
  // This format allow duplicate edges, we need to handle
  static createListAdjWeighted = (ns: NS, file: string) => {
    const g = new Graph<string>();
    const data = ns.read(file);
    let line = "";
    let split: string[] = [];
  ...
  static createDirected = (ns: NS, file: string, w = false, d = false) => {
