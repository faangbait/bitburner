...
export const SpiralizeMatrix = {
    solve(params: number[][]): number[] {
        ...
        while (params.length > 0) {
            solution.push(...params.shift() as number[]);
            try {
    ...
    tests(params: number[][]): number[] {
        [
            ...
            if (!SpiralizeMatrix.arrayEquals(SpiralizeMatrix.solve(t.params), t.solution)) { throw "Test failed to pass" }
        })

        return SpiralizeMatrix.solve(params);
    },

    answer(params: number[][]): number[] { return SpiralizeMatrix.tests(params) }
