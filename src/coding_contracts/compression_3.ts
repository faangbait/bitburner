...
export const LZCompression = {
    ...
    solve(params: string): string {
        ...
        for (let i = 1; i < params.length; i++) {
            ...
            for (let len = 1; len <= 9; len++) {
                ...
                else set(new_state, 0, 1, input + '9' + params.substring(i - 9, i) + '0'); // start new literal
                for (let offset = 1; offset <= Math.min(9, i); offset++) { // start new backreference
                    if (params[i - offset] === c) set(new_state, offset, 1, input + len + params.substring(i - len, i));
                }
            }
            ...
            for (let offset = 1; offset <= 9; offset++) {
                for (let len = 1; len <= 9; len++) {
                    ...
                    for (let new_offset = 1; new_offset <= Math.min(9, i); new_offset++) {
                        if (params[i - new_offset] === c) set(new_state, new_offset, 1, input + len + offset + '0');
                    }
                }
            }
            tmp_state = new_state;
    ...
    tests(params: string): string {
        [
            ...
            if (LZCompression.solve(t.params) !== t.solution) { throw "Test failed to pass" }
        })

        return LZCompression.solve(params);
    },

    answer(params: string): string { return LZCompression.tests(params) }
}
