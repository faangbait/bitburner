...
export const ShortestPathGrid = {
    solve(params: number[][]): string {
        ...
        while (queue.length > 0) {
            ...
            for (const [s, i, j] of [['D', a + 1, b], ['U', a - 1, b], ['R', a, b + 1], ['L', a, b - 1]]) {
                ...
                if (params[i][j] === 1) continue; // We've hit a wall;
    ...
    tests(params: number[][]): string {
        [
            { params: [[0,0],[1,0]], solution: "RD" }, // TODO
        ].forEach(t => {
            if (ShortestPathGrid.solve(t.params) !== t.solution) { throw "Test failed to pass" }
        })

        return ShortestPathGrid.solve(params);
    },

    answer(params: number[][]): string { return ShortestPathGrid.tests(params) }
