...
export const LZDecompression = {
    ...
    solve(params: string): string {
        ...
        while (i < params.length) {
            ...
            if (!isNaN(ref) && type === 1) {
                ...
                pos = i;
                for (; i < len + pos; i++) decoded += params[i];
            }
        }
        return decoded;
    },
    ...
    tests(params: string): string {
        [
            ...
            if (LZDecompression.solve(t.params) !== t.solution) { throw "Test failed to pass" }
        })

        return LZDecompression.solve(params);
    },

    answer(params: string): string { return LZDecompression.tests(params) }
}
